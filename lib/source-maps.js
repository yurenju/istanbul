...
function rawMapFromUrl(url, base) {
    ...
    if (url.slice(0, dataUrlPrefix.length).toLowerCase() === dataUrlPrefix) {
        return new Buffer(url.slice(dataUrlPrefix.length), 'base64').toString();
    }
...
SourceMapCache.prototype = {
    addUrl: function (file, sourceMappingUrl) {
        ...
        if (contents) {
            try {
                console.error(JSON.stringify(JSON.parse(contents),null,4));
    ...
    addRawMap: function (file, sourceMapping) {
        console.error('Add source mapping for: ' + file);
