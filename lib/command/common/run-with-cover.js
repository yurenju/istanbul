...
function run(args, commandName, enableHooks, callback) {
    ...
    var template = {
        ...
        runFn,
        excludes,
        sourceMapCache = sourceMaps.createCache();
        ...
        matcherFor({
            ...
            function (err, matchFn) {
                    ...
                    transformer = function (code, filename) {
                        try {
                            var ret = instrumenter.instrumentSync(code, filename),
                                u = instrumenter.lastOriginSourceMapUrl();
                            if (u) {
                                sourceMapCache.addUrl(filename, u);
                            }
                            return ret;
                        } catch (ex) {
                ...
                process.once('exit', function () {
                    ...
                    collector = new Collector({ transformer: sourceMapCache.transformer()});
                    collector.add(cov);
                    collector.done();
                    if (config.reporting.print() !== 'none') {
